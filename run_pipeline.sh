...
DISTILL_DATASET_PATH="${DISTILL_DATASET_PATH:-openwebtext}"
...
DISTILL_OPENWEBTEXT_EVAL_RATIO="${DISTILL_OPENWEBTEXT_EVAL_RATIO:-0.05}"
...
DISTILL_ONLINE_TRAIN_PACKING="${DISTILL_ONLINE_TRAIN_PACKING:-1}"  # 1|0
DISTILL_STREAMING="${DISTILL_STREAMING:-0}"  # 1|0
...
DISTILL_CMD=(
  ...
  --openwebtext_eval_ratio "${DISTILL_OPENWEBTEXT_EVAL_RATIO}"
...
if [[ "${DISTILL_ONLINE_TRAIN_PACKING}" == "1" ]]; then
  DISTILL_CMD+=(--online_train_packing)
fi
